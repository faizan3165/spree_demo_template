...
Spree.routes.api_v2_storefront_cart_apply_coupon_code = Spree.localizedPathFor(
	'api/v2/storefront/cart/apply_coupon_code'
);
Spree.routes.api_v2_storefront_cart_remove_coupon_code = function(couponCode) {
	return Spree.localizedPathFor('api/v2/storefront/cart/remove_coupon_code/' + couponCode);
};
Spree.routes.product = function(id) {
	return Spree.localizedPathFor('products/' + id);
};
Spree.routes.product_related = function(id) {
	return Spree.localizedPathFor('products/' + id + '/related');
};
Spree.routes.product_carousel = function(taxonId) {
	return Spree.localizedPathFor('product_carousel/' + taxonId);
};
Spree.routes.set_locale = function(locale) {
	return Spree.pathFor('locale/set?switch_to_locale=' + locale);
};
Spree.routes.set_currency = function(currency) {
	return Spree.pathFor('currency/set?switch_to_currency=' + currency);
};
